...
def random_split(x):
    ...
    left_ids = np.random.choice(np.array([0, 1],dtype=np.int64), len(x)) > 0
...
def find_best_split(node, tree, random_state):
    ...
    vars = np.random.choice(np.arange(tree.X.shape[1], dtype=np.int64), tree.max_features, replace=False)
    X = tree.X[node.idx, :]
    y = tree.y[node.idx]
...
def categorical_split(x, y, idx, var, total_counts, tree):
    ...
    if tree.n_classes == 2:
        ...
        categories = np.arange(tree.ncat[var],dtype=np.int64)
        n_splits = int(2**(categories.shape[0]-1))

    for i in range(1, n_splits):
        if tree.n_classes == 2:
            split = np.array([categories[j] for j in range(categories.shape[0]) if j < i],dtype=np.int64)
        else:
            split = np.array([categories[j] for j in range(categories.shape[0]) if (i & (1 << j))],dtype=np.int64)

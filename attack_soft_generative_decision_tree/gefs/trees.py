...
def evaluate_instance(node, x):
    ...
    if not np.isnan(x[s.var]):
        ...
        if s.type == 'num':
            ...
            go_left = np.any(isin_nb(np.array(x[s.var],dtype=np.int64), s.threshold))
    else:
...
def add_split(tree_node, pc_node, ncat, smoothing, root=False):
    ...
    if ncat[split_col] > 1:
        ...
        ind1 = Leaf(scope=np.array([split_col],dtype=np.int64), n=n_points_left+lp,
        ...
        ind2 = Leaf(scope=np.array([split_col],dtype = np.int64), n=n_points_right+lp,
...
def add_dist(tree_node, pc_node, data, ncat, learnspn, max_height, thr,
    ...
    if n_points >= learnspn:
        ...
        for var in scope:
            if ncat[var] > 1:
                leaf = MultinomialLeaf(np.array([var],dtype=np.int64), n_points+lp)
                pc_node.add_child(leaf)
                fit_multinomial(leaf, data_leaf, int(ncat[var]), smoothing)
            else:
                leaf = GaussianLeaf(np.array([var],dtype=np.int64), n_points+lp)
                pc_node.add_child(leaf)
...
def tree2pc(tree, learnspn=np.Inf, max_height=1000000, thr=0.01,
    ...
    ncat = np.array(tree.ncat,dtype=np.int64)
    lp = np.sum(np.where(ncat==1, 0, ncat)) * smoothing # LaPlace counts
...
class Tree:
    ...
    def predict_proba(self, X, smoothing=1e-6):
        ...
        res = np.empty(counts.shape, dtype=np.float64)
        for i in range(counts.shape[0]):
